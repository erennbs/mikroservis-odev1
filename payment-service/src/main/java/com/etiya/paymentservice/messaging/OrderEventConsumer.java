 ...
 * {@code orderCreated-in-0} (Kafka topic "order-created") in the config-server config.
 *
 * <p>The message is consumed as a {@link Message} (not just the payload) so the idempotency key can
 * be read from Debezium's {@code eventId} header. Actual processing — with the Inbox de-duplication —
...
public class OrderEventConsumer {
    ...
    /**
     * Header carrying the source outbox row's id (UUID) — stable across redeliveries, so it is the
     * correct de-duplication key. Deliberately NOT "id": Spring Messaging reserves the "id" (and
     * "timestamp") header names and overwrites them with a framework-generated value that changes on
     * every delivery, which would defeat de-duplication. Debezium emits this via
    ...
    /**
     * Resolves the idempotency key from the Debezium {@code eventId} header. The Kafka header value
     * can arrive as a String or as raw {@code byte[]}, so both are handled. If the header is missing
     * for any reason, we fall back to a deterministic business key ({@code OrderCreated:<orderId>})
     * so de-duplication still works — OrderCreated is emitted once per order.
     */
